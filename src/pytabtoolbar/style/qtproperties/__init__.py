...
from PyQt5.QtCore import QObject, pyqtProperty, pyqtSignal  # type: ignore
...
class PropertyMeta(type(QObject)):
    ...
    def __new__(cls, name, bases, attrs):
        for key in list(attrs.keys()):
            ...
            attrs[key] = _PropertyImpl(type_=type_, name=key, notify=notifier)
...
class _PropertyImpl(pyqtProperty):
    """Property implementation: gets, sets, and notifies of change."""

    def __init__(self, type_, name, notify):
        super().__init__(type_, self.getter, self.setter, notify=notify)  # type: ignore
        self.name = name
